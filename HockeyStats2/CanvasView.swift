...
struct CanvasView: View {
    ...
    var body: some View {
        Canvas { context, size in
            let frame = CGRect(x: 110, y: 90, width: 900, height: 900)
            for shot in viewModel.shots {
                if shot.0.x > frame.minY &&
                    shot.0.x > frame.minX &&
    ...
    .background(Image("halfRink")
        .frame(width: 800, height: 1250, alignment: .top))
    }
}
